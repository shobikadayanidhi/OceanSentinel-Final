/*
  EmsolIoTx.h - Library API for sending data to the Emsol IoTx
...
class Telemetry {
    ...
    friend class EmsolIoTxSized;
#ifndef ESP8266
    template<size_t PayloadSize, size_t MaxFieldsAmt, typename Logger>
    friend class EmsolIoTxHttpSized;
#endif
  public:
    inline Telemetry()
...
// EmsolIoTxSized client class
template<size_t PayloadSize = Default_Payload,
         size_t MaxFieldsAmt = Default_Fields_Amt,
         typename Logger = EmsolIoTxDefaultLogger>
class EmsolIoTxSized
{

    bool provision_mode = false;

  public:
    // Initializes EmsolIoTxSized class with network client.
#if defined(ESP8266) || defined(ESP32)
    inline EmsolIoTxSized(Client &client)
      : m_client(client)
      , m_requestId(0)
      , m_fwVersion("")
      , m_fwTitle("")
    ...
    // Destroys EmsolIoTxSized class with network client.
    inline ~EmsolIoTxSized() { }

    // Connects to the specified Emsol IoTx server and port.
    ...
    // Disconnects from Emsol IoTx. Returns true on success.
    ...
    // Sends telemetry data to the Emsol IoTx, returns true on success.
    template<class T>
    ...
    // Sends integer telemetry data to the Emsol IoTx, returns true on success.
    inline bool sendTelemetryInt(const char *key, int value) {
      return sendKeyval(key, value);
    }

    // Sends boolean telemetry data to the Emsol IoTx, returns true on success.
    inline bool sendTelemetryBool(const char *key, bool value) {
      return sendKeyval(key, value);
    }

    // Sends float telemetry data to the Emsol IoTx, returns true on success.
    inline bool sendTelemetryFloat(const char *key, float value) {
      return sendKeyval(key, value);
    }

    // Sends string telemetry data to the Emsol IoTx, returns true on success.
    inline bool sendTelemetryString(const char *key, const char *value) {
      return sendKeyval(key, value);
    }

    // Sends aggregated telemetry to the Emsol IoTx.
    inline bool sendTelemetry(const Telemetry *data, size_t data_count) {
      return sendDataArray(data, data_count);
    }

    // Sends custom JSON telemetry string to the Emsol IoTx.
    ...
    bool RPC_Subscribe(const RPC_Callback *callbacks, size_t callbacks_size) {
      ...
      if (EmsolIoTxSized::m_subscribedInstance) {
        return false;
      }
      ...
      EmsolIoTxSized::m_subscribedInstance = this;
      for (size_t i = 0; i < callbacks_size; ++i) {
        m_rpcCallbacks[i] = callbacks[i];
      }

      m_client.setCallback(EmsolIoTxSized::on_message);

      return true;
    }

    inline bool RPC_Unsubscribe() {
      EmsolIoTxSized::m_subscribedInstance = NULL;
  ...
  private:
    ...
    // Sends array of attributes or telemetry to Emsol IoTx
    ...
    static EmsolIoTxSized *m_subscribedInstance;
    ...
    static void on_message(char* topic, uint8_t* payload, unsigned int length) {
      ...
      if (strncmp("v1/devices/me/rpc", topic, strlen("v1/devices/me/rpc")) == 0) {
        EmsolIoTxSized::m_subscribedInstance->process_rpc_message(topic, payload, length);
      } else if (strncmp("v1/devices/me/attributes", topic, strlen("v1/devices/me/attributes")) == 0) {
        EmsolIoTxSized::m_subscribedInstance->process_shared_attribute_update_message(topic, payload, length);
      } else {
        if (strncmp("/provision/response", topic, strlen("/provision/response")) == 0) {
#if defined(ESP8266) || defined(ESP32) || defined(ARDUINO_AVR_MEGA)
          EmsolIoTxSized::m_subscribedInstance->process_provisioning_response(topic, payload, length);
#endif
        } else if (strncmp("v2/fw/response/", topic, strlen("v2/fw/response/")) == 0) {
#if defined(ESP8266) || defined(ESP32)
          EmsolIoTxSized::m_subscribedInstance->process_firmware_response(topic, payload, length);
#endif
    }
    }
    }
...
template<size_t PayloadSize, size_t MaxFieldsAmt, typename Logger>
EmsolIoTxSized<PayloadSize, MaxFieldsAmt, Logger> *EmsolIoTxSized<PayloadSize, MaxFieldsAmt, Logger>::m_subscribedInstance;

#if !defined(ESP8266) && !defined(ESP32)

// Emsol IoTx HTTP client class
template<size_t PayloadSize = Default_Payload,
         size_t MaxFieldsAmt = Default_Fields_Amt,
         typename Logger = EmsolIoTxDefaultLogger>
class EmsolIoTxHttpSized
{
  public:
    // Initializes EmsolIoTxHttpSized class with network client.
    inline EmsolIoTxHttpSized(Client &client, const char *access_token,
    ...
    // Destroys EmsolIoTxHttpSized class with network client.
    inline ~EmsolIoTxHttpSized() { }
    ...
    // Sends custom JSON telemetry string to the Emsol IoTx, using HTTP.
    inline bool sendTelemetryJson(const char *json) {
      if (!json || !m_token) {
        return  false;
      }
    ...
    // Sends custom JSON with attributes to the Emsol IoTx, using HTTP.
    inline bool sendAttributeJSON(const char *json) {
      if (!json || !m_token) {
        return  false;
      }
...
using EmsolIoTxHttp = EmsolIoTxHttpSized<>;

#endif

using EmsolIoTx = EmsolIoTxSized<>;
